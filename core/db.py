...
def add_topology_link(src_asset_id, dst_asset_id, src_interface="", dst_interface="",
                      link_type="ethernet", bandwidth="", return_created=False):
    ...
    if existing:
        conn.close()
        return (existing[0], False) if return_created else existing[0]
    ...
    conn.close()
    return (link_id, True) if return_created else link_id
