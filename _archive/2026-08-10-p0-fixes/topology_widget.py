...
class TopologyWidget(QWidget):
    ...
    def _import(self):
        ...
        try:
            ...
            allowed_ids = {a["id"] for a in self._current_assets()}
            count = 0
            skipped = 0
            for edge in data.get("edges", []):
                src = edge["src"]
                dst = edge["dst"]
                if self.current_location != "全部站点" and (src not in allowed_ids or dst not in allowed_ids):
                    skipped += 1
                    continue
